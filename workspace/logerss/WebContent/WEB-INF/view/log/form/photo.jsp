...
<div id="photo-form-container" class="panel panel-default" style="margin-bottom:10px;">
	...
	<h3 class="panel-title">
				<div class="input-group input-group-sm" >
		  			...
		  			<input id="photo-form-title"  value="photo" type="text" class="form-control" placeholder="title" aria-describedby="sizing-addon3"/>
				</div>
	</h3>
	</div>
...
function addImg(imgBase64,date,latlgn){
	...
	var latlngId = 'photo-form-content-latlng-'+id;
	var dateId = 'photo-form-content-date-'+id;
	var h="";
	 	 h+='  <div id="'+contentId+'" name="photo-form-content" class="thumbnail">';
		 h+='  <img  name="photo-form-content-photo" src="'+imgBase64+'" width="400">';
		 ...
		 h+='			<input id="'+dateId+'" name="photo-form-content-date" value="'+(date||"")+'"  type="text" class="form-control" placeholder="yyyyMMddHHmmss" aria-describedby="sizing-addon3"/>';
		 h+='			<span class="input-group-addon"  id="sizing-addon3"><span class="fa fa-map" aria-hidden="true"/></span>';
		 h+='			<input id="'+latlngId+'" name="photo-form-content-latlng"  value="'+(latlgn||"")+'"  type="text" class="form-control" placeholder="latitude, longitude" aria-describedby="sizing-addon3"/>';
...
// 	debugger;
	...
	for (var i = 0; i < files.length; i++) {
	    ...
	    EXIF.getData(file, function() {
	    	...
	    	ImageUtil.thumbnailBase64ToBase64(this.getImgBase64(),100,function(bb){
	    		date = date||DateUtil.getDate("yyyyMMddHHmmss");
	    		date = StringUtil.replaceAll(date,":","");
	    		date = StringUtil.replaceAll(date," ","");
		        var id = addImg(bb,date,latlgn);
		        
		        if(!latlgn){
			    	(function(id){
			    		GPSUtil.getGPS(function(geo){
			    			$("#photo-form-content-latlng-"+id).val(geo.latitude+","+geo.longitude);
			    		});
			    	})(id);
		        }
